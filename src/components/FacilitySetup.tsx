...
type FacilityRow = {
	...
	sortOrder: number; // stays internal for drag and drop
};
...
export default function FacilitySetup() {
	...
	useEffect(() => {
		...
		if (
			(data as any)?.facilitySetup &&
			Array.isArray((data as any).facilitySetup)
		) {
			initial = (data as any).facilitySetup;
		} else {
			try {
	...
	const updateRow = (id: number, patch: Partial<FacilityRow>) => {
		setRows((prev) =>
			prev.map((r) => (r.id === id ? normalizeRow({ ...r, ...patch }) : r)),
		);
	};
	...
	const clearAll = () => {
		if (!window.confirm("Clear all cost centers?")) return;
		setRows([]);
	};
	...
	const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			window.alert("There was an error reading your Excel file.");
		} finally {
			setUploading(false);
	...
	const handleFunctionalAreaChange = (row: FacilityRow, value: string) => {
		if (value === "__add_new__") {
			const name = window.prompt("New functional area?");
			if (name && name.trim()) {
	...
	const handleUnitGroupingChange = (row: FacilityRow, value: string) => {
		if (value === "__add_new__") {
			const name = window.prompt("New unit grouping?");
			if (name && name.trim()) {
				const trimmed = name.trim();
								...
								<td className="px-3 py-2 align-middle">
									...
									>
										<input
											...
											onChange={(e) =>
												updateRow(row.id, { isFloatPool: e.target.checked })
											}
										/>
